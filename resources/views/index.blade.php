...
@section('title', 'DEV-ID | Dashboard')
...
<div id="page-content-wrapper">
  ...
  <nav class="navbar navbar-expand-lg navbar-light bg-dark border-bottom">
    ...
    <div class="container-fluid">
      <h2 class="mt-4 text-center">Monitoring Tambak Udang</h2>
    <!-- <hr class="line1"> -->
    </div>
    <!-- @foreach($data_all as $data)
        <p>{{ $data->NTU}}</p>
    ...
    <div class="container">
      <div class="row">
        ...
        <div class="col-md-3">
          <div class="card center" style="width: 14rem;">
            <div class="card-body head-2">
              <h5 class="card-title">doValue : {{end($doValue)}}</h5>
            </div>
            @if(end($doValue) <=3)
            <h5 class="card-title body-text">kurang</h5>
            @elseif(end($doValue) >=7)
            <h5 class="card-title body-text">bagus</h5>
            @else
            <h5 class="card-title body-text">normal</h5>
            @endif
          </div>
        ...
        <div class="col-md-3">
          <div class="card center " style="width: 14rem;">
            <div class="card-body head-3">
              <h5 class="card-title">kadarGaram : {{end($kadarGaram)}}</h5>
            </div>
            @if(end($kadarGaram) <=3)
            <h5 class="card-title body-text">tidak asin</h5>
            @elseif(end($kadarGaram) >=7)
            <h5 class="card-title body-text">asin</h5>
            @else
            <h5 class="card-title body-text">cukup</h5>
            @endif
          </div>
        </div>
      </div>
...
var doValue= <?php echo json_encode($doValue); ?>;
var kadarGaram= <?php echo json_encode($kadarGaram); ?>;
